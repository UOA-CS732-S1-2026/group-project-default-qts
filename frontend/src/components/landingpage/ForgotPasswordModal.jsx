...
export default function ForgotPasswordModal({ onClose }) {
    ...
    const onStep2 = (data) => {
        ...
        const result = resetPassword(foundUser.id, data.email, data.securityAnswer, data.newPassword);
        if (result.success) {
            setSuccess(true);
        } else {
            if (result.emailError) setError2('email', { type: 'manual', message: result.emailError });
            if (result.answerError) setError2('securityAnswer', { type: 'manual', message: result.answerError });
        ...
        <AnimatePresence>
            ...
            >
                ...
                >
                    ...
                    {step === 2 && !success && (
                        <>
                            ...
                            <form onSubmit={handleSubmit2(onStep2)} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                                ...
                                {/* Security Question (auto-populated) */}
                                <div className="field-group">
                    ...
                    {/* ---- Success ---- */} 
                    {/* TODO: success message can be extracted to a separate component */}
                    {step === 2 && success && (
